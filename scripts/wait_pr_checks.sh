...
#   1 - terminal failure (conflicts, failing checks, etc.)
...
if [ ! -x "$CHECK_REVIEWS_SCRIPT" ]; then
  echo "❌ Missing executable helper script: $CHECK_REVIEWS_SCRIPT" >&2
  exit 1
fi
...
CHECK_PR_CHECKS_ONCE() {
  ...
  case "$pr_state" in
    ...
    OPEN) ;;
    *)
      echo "❌ Unexpected PR state '$pr_state' for PR #$PR_NUMBER" >&2
      return 1
      ;;
  esac
  ...
  if [ "$has_fail" -eq 1 ]; then
    ...
    echo "$checks"
    echo ""
    echo "💡 Common local repro commands:"
    echo "   make ci"
    return 1
  fi

  # All checks passed — verify reviews and merge state
  if [ "$has_pass" -eq 1 ] && [ "$has_pending" -eq 0 ] && [ "$has_fail" -eq 0 ]; then
    ...
    case "$merge_state" in
      BLOCKED | DRAFT | HAS_HOOKS | UNKNOWN | UNSTABLE)
        ...
        ;;
      *)
        echo "❌ Checks passed but merge state '$merge_state' is unexpected" >&2
        return 1
        ;;
    esac
  fi
...
if [ "$MODE" = "once" ]; then
  ...
  case "$rc" in
    0 | 1 | 10)
      ...
      ;;
    *)
      echo "❌ Unexpected checks status code '$rc'" >&2
      exit 1
      ;;
  esac
fi
